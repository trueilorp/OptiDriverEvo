...
def mutate(driver):
    ...
    index_of_gen_to_modify = random.randint(low_bound, upper_bound) 
    if 17 <= index_of_gen_to_modify <= 21:
        ...
        while (value_of_modify == 0) or (value_modified < 0 or value_modified > 1): 
            value_of_modify = round(random.uniform(-0.5, 0.5),2) 
            value_modified = value_of_modify + float(gens[index_of_gen_to_modify])
        
        gens[index_of_gen_to_modify] = "{:.2f}".format(value_modified) 
    return gens

def get_best_driver(driver_parent, driver_offspr): 
